...
@router.post("/book/add", tags=tags, response_model=base.RestfulModel[base.AddResult], name="添加书籍和学生")
@decorator
@atomic()
async def book_add(param: book.Add):
    ...
    #record = await models.StudentModel.filter(pk=param.student_id).first()
...
async def book_edit(param: book.Edit):
    ...
    return {
        "result": 1,"name": "spuer python爬虫教学"
    }
...
@router.get("/book/{book_id}/student", tags=["book"], response_model=base.RestfulModel)
@decorator
